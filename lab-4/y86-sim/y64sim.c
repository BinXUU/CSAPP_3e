...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    int sign_a=(int)(argA>>63)&(0x1);
    int sign_b=(int)(argB>>63)&(0x1);
    int sign_val=(int)(val>>63)&(0x1);

    if((op==A_ADD)&&(!(sign_a^sign_b))&&(sign_a^sign_val)){
        ovf=1;
    }

    if((op==A_SUB)&&(sign_a^sign_b)&&(!(sign_a^sign_val))){
        ovf=1;
    }
